...
UA = "Mozilla/5.0 (Linux; rv:128.0) Gecko/20100101 " "Firefox/128.0 decline-bot/3.2"
...
name_col = next(
    (c for c in tbl.columns if c.lower().startswith("agg") or c.lower() == "name"), None
)
if not name_col:
...
domain_cols = [
    c for c in tbl.columns if any(k in c.lower() for k in ("site", "url", "domain"))
]
...
if CSV_OUTPUT.exists():
    ...
    if done:
        log.info(
            "Resume mode: %d brands already done → %d left", len(done), len(AGGREGATORS)
        )
