...
def is_blocked(row):
    ...
    return (
        "bug" in bug
        or "block" in blocked
        or "api error" in status
        or "not working" in status
    )
...
class AggregatorSelector:
    ...
    def select_top(self, country, method, amount, kyc_wanted: bool = True):
        ...
        for reg in regions:
            ...
            if not row.empty:
                ...
                for agg in aggs:
                    if self.check_limits(agg, amount) and self.check_kyc(
                        agg, kyc_wanted
                    ):
                        result.append(agg)
                if result:
        ...
        for alt_method in ["card", "sepa", "bank", "apple pay"]:
            alt = df[
                ...
                & (df["Method"].str.lower() == alt_method)
            ]
            if not alt.empty:
                aggs = [
                    alt.iloc[0][col]
                    for col in ["Top 1", "Top 2", "Top 3"]
                    if pd.notna(alt.iloc[0][col])
                ]
                for agg in aggs:
                    if self.check_limits(agg, amount) and self.check_kyc(
                        agg, kyc_wanted
    ...
    def check_limits(self, aggregator, amount):
        row = self.tbl1[
            self.tbl1["Aggregator"].apply(lambda x: aggregator_match(x, aggregator))
        ]
