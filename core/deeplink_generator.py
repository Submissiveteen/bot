...
class DeeplinkGenerator:
    ...
    def get_query_map(self, aggregator):
        df = self.table_4[self.table_4["aggregator"].str.contains(aggregator.split('.')[0], case=False)]
        return {row["feature"]: row["parameter"] for _, row in df.iterrows() if pd.notna(row["parameter"])}
    ...
    def generate(self, aggregator, base_url, params_dict):
        ...
        missing = [field for field in required if field not in params_dict or params_dict[field] is None]
        if missing:
            raise ValueError(f"Missing required fields for {aggregator}: {missing}")
        query = {query_map.get(k, k): v for k, v in params_dict.items() if k in query_map and v is not None}
